...
def plot_drugbank_approved(data_path: Path, save_dir: Path, top_k_atc_codes: int = 25) -> None:
    """Plot statistics about the DrugBank approved molecules.
    ...
    for level in trange(1, 5, desc="ATC levels"):
        ...
        atc_code_counts = Counter(
            atc_code for atc_list in data[atc_column].dropna() for atc_code in atc_list.split(DRUGBANK_DELIMITER)
        )
        atc_code_df = pd.DataFrame.from_dict(atc_code_counts, orient="index", columns=["count"])
        ...
        plt.savefig(save_dir / f"atc_level_{level}_distribution.pdf", bbox_inches="tight")
        plt.close()
